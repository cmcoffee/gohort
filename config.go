...
func init_database() {
	...
	// Load persisted STT config for audio transcription (used by
	// download_video to add a transcript alongside sampled frames).
...
func init_logging() {
	...
	if global.trace || global.snoop {
		setupTraceFile(logs_dir)
	}
	if global.snoop {
...
// enable_trace enables trace output to stdout AND routes the file
// sink to the dedicated trace log (created on demand if init_logging
// hasn't already opened it). Mirrors init_logging's split so a flag
// set after the top-level parse still gets the same routing.
func enable_trace() {
...
// setupTraceFile opens (or reuses, if already opened this run) the
// dedicated trace log file and routes the TRACE level at it. Safe
...
func setupTraceFile(logs_dir string) {
	...
	if err != nil {
		...
		// of silently dropping it — better noisy log than no log.
		Err("trace log open failed (%v); trace output will land in %s.log instead", err, APPNAME)
