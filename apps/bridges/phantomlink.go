...
// phantomLinkImpl makes Bridges the PhantomLink provider — the agent-facing seam
...
type phantomLinkImpl struct{ T *Bridges }
...
func (p phantomLinkImpl) convoSummary(c Convo) PhantomChatSummary {
	return PhantomChatSummary{
		ChatID:      c.ChatID,
		Handle:      c.Handle,
...
func (p phantomLinkImpl) deliver(owner, chatID, handle, text string, images []string) error {
...
func (p phantomLinkImpl) ResolveRecipient(owner, to string) (PhantomChatSummary, bool) {
	...
	if looksLikeHandle(to) {
		return PhantomChatSummary{Handle: to, DisplayName: to}, true
	}
	return PhantomChatSummary{}, false
}
