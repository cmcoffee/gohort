	...
	return AgentToolDef{
		Tool: Tool{
			...
			Parameters: map[string]ToolParam{
				...
				"format_script":    {Type: "string", Description: "condition=change, optional: sandboxed python to format the alert (e.g. \"X joined\"). On stdin you get {\"prior\":...,\"current\":...,\"prior_status\":...,\"current_status\":...}: prior/current are the tool's body with the \"HTTP <code>\" status line split off so json.loads works directly, and *_status hold that status line if you want to react to errors. Print the alert to stdout; if it errors or prints nothing the built-in diff is used. Omit to just use the built-in diff."},
