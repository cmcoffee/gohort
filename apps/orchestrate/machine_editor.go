	...
	for _, p := range def.Phases {
		...
		out = append(out, ui.Stack{Children: []ui.Component{
			...
			ui.Toolbar{Actions: []ui.ToolbarAction{{
				...
				Label:  "↑ Move up",
				Title:  "Move " + p.Name + " one place earlier. Order is the reading order and which waiting step catches a step that hands off nowhere. In the map it only swaps steps that sit at the same depth — the shape there follows the arrows, not the list.",
				Method: "client",
				URL:    "machine_move_step",
				Data:   p.Name + "|up",
			}, {
				Label:  "↓ Move down",
				Title:  "Move " + p.Name + " one place later. Order is the reading order and which waiting step catches a step that hands off nowhere. In the map it only swaps steps that sit at the same depth — the shape there follows the arrows, not the list.",
				Method: "client",
				URL:    "machine_move_step",
				Data:   p.Name + "|down",
			}, {
