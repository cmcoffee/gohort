...
func TestTheToolDecodesAndDocumentsEveryPhaseField(t *testing.T) {
	...
	one := map[string]any{
		...
		"reach": "read", "tools": []any{"read_file"}, "deny": []any{"web_search"},
		"model": "lead", "think": "on",
		...
		"tool": "http_get", "args": map[string]any{"url": "{state:plan.url}"},
		"accumulates": []any{map[string]any{"name": "answers", "from": "parts", "mode": "append"}},
	}
