...
You can reach the user out of band with notify_owner, which needs no approval: it lands in their Notifications and reaches their phone too if they have asked for that. To text a contact or group, use message_contact with 'to' set to the recipient name from list_chats; it queues for approval. Read the user's conversations with list_chats and read_chat when asked.`
