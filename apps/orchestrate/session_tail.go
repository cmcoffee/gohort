...
func init() {
	RegisterTunable(TunableSpec{
		...
		Help:     "How many of a conversation's most recent messages are sent to the browser on open.",
		Detail:   "Older ones stay stored and stay in the agent's context. This only bounds what the page renders at once, so a long-running channel thread opens quickly instead of building thousands of bubbles. \"Load earlier\" fetches more, and 0 loads the whole thread every time.",
		Kind:     KindInt,
		Default:  80,
		Min:      0,
		Max:      100000,
	...
	RegisterTunable(TunableSpec{
		...
		Help:     "The same bound for an agent's standing thread, its cortex.",
		Detail:   "A cortex is append-only and never ends, so it outgrows an ordinary conversation by a wide margin and is the one that opens slowly. Its cards are pointers whose value is recency, and the first screen is what anyone actually reads. \"Load earlier\" fetches more, and 0 falls back to the general thread limit.",
		Kind:     KindInt,
		Default:  30,
		Min:      0,
		Max:      100000,
