...
func (t *chatTurn) entityRelatedPassages(e GraphEntity) string {
	...
	defer cancel()
	hits := searchAgentKnowledgeVec(ctx, t.app.DB, t.user, t.ownerUser, t.readsOwnerCorpus(ChunkScopeAll), t.agent.ID, generalTopic, query, t.embedQuery(ctx, query), 3, t.skillsActive, t.agent.AttachedCollections, ChunkScopeAll)
