	...
	if !subAgent {
		fields = append(fields,
			...
			ui.FormField{Field: "channel", Type: "toggle", Label: "Maintain a Cortex thread",
				Help: "Gives the agent a persistent Cortex thread (its mind — the 🧠 row pinned at the top of the rail, above its ordinary sessions) where event-monitor wakes and standing-agent reports land, kept bounded by a rolling summary. It also surfaces the Permissions queue and the Manage menu in the topbar. Can be published to the dashboard (each granted user gets their own private Cortex thread + compaction) as long as the delegation & management tools (below) are off."},
			ui.FormField{Field: "share_cortex_knowledge", Type: "toggle", Label: "Share Cortex awareness with granted users", ShowWhen: "channel",
				Help: "Off (default): a granted user on the dashboard gets a blank, isolated thread — the agent answers as a fresh clone, unaware of its Cortex. On: their answers are informed by the agent's REAL Cortex awareness (its recent channel feed + monitor fires), read-only — so it 'knows the things' the agent knows. They still cannot open, read, or manage the Cortex thread itself. Leave OFF for personal agents fed by private channels (iMessage, etc.); turn ON for shared/team agents whose accumulated awareness is meant to help the people you grant access to."},
