...
Tool (no LLM call, no tokens): tool=<name> + args={param: template}. For computation, not judgment — arithmetic, dedup, normalization, one API call.

Model tier: a worker stage's model="lead" runs it on the precision tier (default "worker"). Use it on decompose / synthesize / judge stages only.

Branch (no LLM call): when=<stage>.<bool field> + optional skip_to=<later stage>; without skip_to it ends the pipeline. Forward-only.
