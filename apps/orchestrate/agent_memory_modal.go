    ...
    window.uiRegisterClientAction('__UI_ACTION__', function() {
      ...
      function loadAudit() {
        fetch(MEMBASE + 'memaudit').then(function(r){ return r.ok ? r.json() : null; }).then(function(d) {
          ...
          renderAudit((d && d.findings) || [], (d && d.ignored) || []);
        }).catch(function(){});
      }
      // actOnAudit runs Remove, Ignore or Restore on one finding. The server finds
      // the finding again by its id and acts on what IT found, so the page never
      // names what to delete.
      function actOnAudit(action, f, btn) {
        (async function() {
          if (action === 'remove' && !(await window.uiConfirm(f.remove + '\n\n“' + (f.quote || '') + '”'))) return;
          btn.disabled = true;
          fetch(MEMBASE + 'memaudit', {method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({action: action, id: f.id})})
            .then(function(r) { if (!r.ok) return r.text().then(function(t) { throw new Error(t || ('HTTP ' + r.status)); }); })
            .then(function() { loadAudit(); })
            .catch(function(e) { btn.disabled = false; window.uiAlert('Could not ' + action + ' it: ' + e.message); });
        })();
      }
      ...
      function renderAudit(found, ignored) {
        auditList.innerHTML = '';
        if (!found.length && !ignored.length) { auditWrap.style.display = 'none'; return; }   // silent when clean
        auditWrap.style.display = '';
        auditTitle.textContent = found.length ? 'Needs attention (' + found.length + ')' : 'Needs attention';
        found.forEach(function(f) {
          ...
          if (f.id) {
            ...
            acts.style.cssText = 'display:flex;flex-wrap:wrap;gap:0.35rem;margin-top:0.35rem';
            if (f.remove) {
              acts.appendChild(auditButton('Remove', f.remove, function(b) { actOnAudit('remove', f, b); }));
            }
            acts.appendChild(auditButton('Ignore', 'Stop showing this until the text behind it changes', function(b) { actOnAudit('ignore', f, b); }));
