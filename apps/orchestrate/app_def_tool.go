	...
	return AgentToolDef{
		Tool: Tool{
			...
			Parameters: map[string]ToolParam{
				...
				"script": {
					Type:        "string",
					Description: "(patch_html/replace_function/get) Edit a SCRIPT instead of an html section: the name of a data source or action (\"data:<name>\" / \"action:<name>\" when one name is used for both). With get, returns that script's full body — read it before patching. patch_html then does exact find/replace inside the script; replace_function swaps one named Python def (or bash function) for the whole new one. Never re-send the data_sources/actions array to change one line of one script.",
				},
				"note": {
					Type:        "string",
					Description: "(create/update/patch_html/replace_function/revert) One line on WHY this edit was made (\"added a city filter\", \"fixed the empty-array bug\"). Stored with the revision and shown in revisions and get, so a later session — or a different agent — can tell a deliberate choice from an accident before undoing it. Always pass one on an edit that changes behavior.",
				},
				"to": {
					Type:        "string",
				...
				"sample": {
					Type:        "array",
					Description: "(test/verify) Example form submissions to run the data sources/actions against, standing in for the live record store. KEPT on the app: later tests, verifies and every save's auto-check reuse it while the store is empty, so pass it once. Keys are checked against the record fields the sections declare — a key no section names is flagged. Each item is an object keyed by the FORM's field names — exactly what a record looks like after the user submits the form (e.g. [{\"city\":\"Santa Cruz, CA\"}]). Use this to test end-to-end before the app has any real records: the scripts receive these as the `records` env var, so you see whether 'add a location → forecast' actually produces output. If a data source returns [] against a sample that clearly should match, the script isn't reading the records env var (or has the wrong field name).",
				...
				"description": {Type: "string", Description: "(create/update) One-line summary of what the app is for (shown on the My Apps index)."},
				"notes":       {Type: "string", Description: "(create/update) The app's standing notes for whoever revises it next — you, in a later session, or another agent: what it is for, the decisions made and WHY (\"record_key is email because entries are per person\"), and what the owner asked for that is not done yet. REWRITE the whole text when it changes (omit it to keep what is stored; send \"\" to clear). Capped at 3000 characters — a summary, not a log; per-edit reasons go in `note`. Read it back with get before changing an app you did not just build."},
...
- get  {id(slug), script?} — one app's full section definition, its standing notes, its revision + change note, its verify status, its record fields (derived from the sections) next to the live record count and any stored fields the sections no longer name, its retained sample, and each script's size and function names (script=<name> returns that one script's body). Read it before revising an app you did not just build.
