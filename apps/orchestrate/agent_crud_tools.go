...
func agentMutationParams(includeID bool) map[string]ToolParam {
	params := map[string]ToolParam{
		...
		"disable_inferred":  {Type: "boolean", Description: "Optional. When true, turn off the Reference Memory layer — the vector-grown derived store (memory_save findings, synthesis auto-ingest, closer findings). memory_save / memory_search / memory_forget stripped from catalog; synthesis auto-ingest skipped; skills classifier suppressed for the turn (skills emit derived chunks via self-training). Use for agents that should answer from authoritative sources only and never grow their own fuzzy recall (KB readers, compliance bots). The per-turn Clean toggle on the chat surface is the same switch scoped to a single turn. Default false."},
