...
func agentMutationParams(includeID bool) map[string]ToolParam {
	params := map[string]ToolParam{
		...
		"evals": {
			Type:        "array",
			Description: "Optional saved test cases for the eval harness. Each entry is a EvalCase: {name, prompt, must_include[], must_not_include[], must_call_tools[], must_not_call_tools[], stub_results{}, judge_prompt, notes}. The harness runs each case as an independent fresh session, captures the agent's reply AND the tools it called, and grades: must_include / must_not_include are case-insensitive substring checks on the reply; must_call_tools / must_not_call_tools check the actual TOOL-CALL trace (catches a model that describes an action but never emits the tool call — e.g. must_call_tools=[\"await_result\"] for a deferred-reply scenario); judge_prompt (optional) is a criterion an LLM judge grades the reply against (PASS/FAIL + reason). stub_results maps a tool name to a canned result string, used in STUB mode (the DEFAULT) so tools don't run for real — the model sees the scripted output and no message is sent / monitor created / network hit. Use to lock in behaviors before a prompt edit: a chat agent's \"asks_clarifying\" case might have must_include=[\"?\"]; an orchestrator's \"awaits_reply\" case might have must_call_tools=[\"message_contact\",\"await_result\"]. Run via POST .../api/agents/{id}/eval?runs=30 — runs each case 30x and reports the pass RATE (a single run of a local model is noise). Eval is STUB-by-default (nothing real fires); add ?live=1 to run real non-consequential tools, or ?live=all to also allow consequential (approval-gated) tools to fire for real.",
			Items:       &ToolParam{Type: "object"},
		},
