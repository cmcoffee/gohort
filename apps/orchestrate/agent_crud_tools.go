...
func agentMutationParams(includeID bool) map[string]ToolParam {
	params := map[string]ToolParam{
		...
		"think_budget":             {Type: "integer", Description: "Optional max thinking tokens per LLM call. 0 (default) = deployment default (4096). The admin global budget is a hard ceiling, so this can only LOWER it. Applies only when thinking is on."},
		"lead_model":               {Type: "boolean", Description: "Optional. When true, MAIN reasoning (plan + synthesis) escalates to the lead/precision LLM; per-step worker phases stay on the worker. Degrades to worker when no distinct lead is configured, and never escalates when force_private or the turn is Private-toggled. Use where planning quality justifies the remote cost. Default false."},
		...
		"disable_explicit":         {Type: "boolean", Description: rewriteMemoryToolNames("Optional. When true, turns off Explicit Memory (store_fact / list_facts / forget_fact + the always-in-prompt facts block). For impersonal agents that should hold no standing state (KB readers, one-shot transformers). Orthogonal to disable_inferred. Default false.")},
		"disable_inferred":         {Type: "boolean", Description: rewriteMemoryToolNames("Optional. When true, turns off Reference Memory (the vector store grown via memory_save): memory_save / memory_search / memory_forget stripped from the catalog, derived chunks excluded from recall. For agents that must answer from authoritative sources only. The per-turn Clean toggle is this same switch scoped to one turn. Default false.")},
		"memory_mode":              {Type: "string", Description: rewriteMemoryToolNames("Optional Explicit Memory framing: \"agent\" (default) or \"chatbot\". agent = store_fact holds generalized lessons only (design principles, recurring gotchas); specifics go to Reference Memory via memory_save. chatbot = those lessons PLUS user personalization and conversation-coherence notes. chatbot for general-purpose conversational agents, agent for task-focused ones. No-op when disable_explicit is true.")},
		"enable_notes":             {Type: "boolean", Description: "Optional. Turns on the Working-notes layer: one bounded, agent-REWRITABLE block of running state, always in prompt, plus the update_notes tool. Distinct from store_fact (append-only durable rules) — notes are a scratchpad rewritten wholesale as task state changes. Opt in for long-running conversational/project agents; leave off for stateless ones. Default false."},
		"seed_notes":               {Type: "string", Description: "Optional initial Working-notes text (requires enable_notes; no-op otherwise). Rendered and rewritable from the first turn; stays the durable fallback until the first update_notes. Keep under 1500 characters."},
		"allow_private_mode":       {Type: "boolean", Description: "Optional. Exposes a per-turn Private toggle on the public surface that drops internet-capability tools. Opt in only where local-only operation is meaningful. Default false."},
		"force_private":            {Type: "boolean", Description: "Optional. Locks the agent into Private mode: every turn drops network-capability tools (web_search, fetch_url, dispatch, …) regardless of the user toggle, and the toggle is hidden. For agents that must NEVER reach the network. Overrides allow_private_mode. Default false."},
		"disable_skills":           {Type: "boolean", Description: "Optional. When true, skills are fully suppressed: no activation, no prompt addendum, no skill_knowledge chunks, no skill-attached tools. For agents that must faithfully report one source (KB readers, compliance look-ups). The per-turn Clean toggle also suppresses skills. Default false."},
		"allowed_skills":           {Type: "array", Description: "Strict allowlist of skill IDs the classifier may consider. Every skill is opt-in per agent; empty (default) = none active. IDs from skill_def(action=list).", Items: &ToolParam{Type: "string"}},
		"hidden":                   {Type: "boolean", Description: "Optional. When true, hidden from other agents' \"Available agents\" block and refused by agents(run) — unless a caller lists it in allowed_dispatch_targets. For personal direct-chat agents the fleet shouldn't route to. Default false."},
		"allowed_dispatch_targets": {Type: "array", Description: "Optional dispatch allowlist of agent IDs. Empty (default) = may call any non-hidden agent. Non-empty = ONLY the listed targets, hidden or not — the explicit pick wins, so this also reaches hidden specialists.", Items: &ToolParam{Type: "string"}},
		"attached_collections":     {Type: "array", Description: "Optional Document Collection IDs merged into this agent's RAG recall — a curated reference corpus without authoring a skill. Same many-to-many shape as skills, bound at the agent layer (no activation needed). IDs from the Collections surface. Default empty.", Items: &ToolParam{Type: "string"}},
		"attached_pipelines":       {Type: "array", Description: "Optional pipeline IDs (pipeline action=list). Each becomes its own callable tool on this agent (run_<pipeline>), so a saved multi-stage workflow is on hand without the generic pipeline tool. Author the pipeline first if it doesn't exist. Default empty.", Items: &ToolParam{Type: "string"}},
		"recall_hints":             {Type: "boolean", Description: "Optional. Each turn surfaces a short scored list of the agent's OWN knowledge relevant to the message — pointers (title + doc_id for fetch_knowledge_doc), not content. Best with a real corpus. Thresholds are deployment tunables. Default false."},
		"triggers":                 {Type: "array", Description: "Optional substring/glob patterns matched against each user message. On a match the host agent gets a per-turn nudge to dispatch HERE first — stronger than the static catalog where the host has priors. Author SPECIFIC patterns the domain's questions actually contain (criminal law: \"penal code\", \"felony\", \"sentencing\"); loose patterns over-fire and train the host to ignore the hint. Empty = in the catalog, no nudge.", Items: &ToolParam{Type: "string"}},
		"owned_by":                 {Type: "string", Description: "Optional parent agent ID making this a sub-agent: deleting the parent cascade-deletes this agent (sessions/memory/knowledge included), and the parent can dispatch to it via agents(action=\"run\") without an allowed_dispatch_targets entry — ownership IS the dispatch link. Combine with hidden=true to keep it out of the global fleet menu while reachable from its parent."},
		"ingest_attachments":       {Type: "boolean", Description: "Optional. Extracted text from uploaded documents (PDF/DOCX/text) is ALSO ingested into the agent's knowledge store under topic=\"attachments\", retrievable by knowledge_search in later sessions. For document-Q&A / reviewer agents whose uploads are referenced repeatedly. Default false."},
		"think":                    {Type: "string", Description: "Optional reasoning override: \"on\", \"off\", or \"auto\" (the framework route decides). CREATE defaults: top-level agents \"on\", sub-agents (owned_by set) \"off\". UPDATE preserves the stored value when omitted. \"on\" for planners/synthesizers; \"off\" for lookup-shaped specialists, transformers, routers."},
		"intake_form": {
			Type:        "array",
			Description: "Optional intake form, shown on the first turn of every new session (chat input hidden until submitted). Values pack into a markdown user message; file fields upload as attachments (PDF/DOCX text-extracted server-side, images to vision). Each entry: {name, label, type, placeholder, help, required, options, allow_other}. type: \"text\" (default), \"textarea\", \"select\" (exactly one), \"checklist\" (any combination, comma-joined), \"number\", \"file\", \"button\" (clicking submits immediately with the label as value). options feeds select/checklist/button; allow_other (checklist only) adds an \"Other:\" free-text row. Use when the agent always needs structured input up front; omit for chat-first agents.",
			Items:       &ToolParam{Type: "object"},
		},
		"tools": {
			Type:        "array",
			Description: "Optional agent-scoped tools that auto-load whenever this agent runs — bespoke shell/api tools for THIS agent's job, kept out of the user-wide pool (two agents can carry same-named tools with different configs). Each entry a TempTool: {name, description, params, mode (\"shell\"|\"api\"), command_template, body_template, credential, method}. Do NOT also list these in allowed_tools; they attach automatically. For a multi-stage workflow author a pipeline and attach it via attached_pipelines, not a tool.",
			Items:       &ToolParam{Type: "object"},
		},
		"evals": {
			Type:        "array",
			Description: "Optional saved test cases for the eval harness. Each EvalCase: {name, prompt, must_include[], must_not_include[], must_call_tools[], must_not_call_tools[], stub_results{}, judge_prompt, notes}. Each runs as a fresh session: case-insensitive substring checks on the reply; tool checks against the ACTUAL call trace (catches narrated-but-never-emitted calls); judge_prompt an optional LLM-judged criterion; stub_results tool→canned result. STUB is the default — nothing real fires. Run via POST .../api/agents/{id}/eval?runs=30 (?live=1 non-consequential for real, ?live=all everything). Locks in behavior before prompt edits.",
			Items:       &ToolParam{Type: "object"},
		},
