...
// At most maxVisionPages pages are processed to bound latency. Routes through
// T.WorkerChat so vision work stays on the local worker tier (app.servitor is
// Private — appliance content must never leave the local LLM).
func extractPDFScreenshots(ctx context.Context, T *Servitor, data []byte) string {
	if T == nil {
		return ""
	}
...
func enrichSupplementWithScreenshots(ctx context.Context, T *Servitor, udb Database, wsID, suppID string, pdfData []byte) {
	...
	screenshots := extractPDFScreenshots(ctx, T, pdfData)
...
func (T *Servitor) handleSupplementAdd(w http.ResponseWriter, r *http.Request) {
	...
	if strings.ToLower(filepath.Ext(fh.Filename)) == ".pdf" {
		go enrichSupplementWithScreenshots(AppContext(), T, udb, wsID, supp.ID, data)
	} else {
