...
const (
	...
	// RiskUnverified is everything the gate cannot confirm is read-only: a
	// program it does not recognize, a script or interpreter whose code it
	// cannot read, shell syntax it cannot see through. The classifier used to
	// call all of that RiskNone, which is what "fails open" meant. It is a
	// category rather than a hard refusal so an operator who trusts a box can
	// grant it, and it ranks BELOW the named categories, so a line that also
	// deletes or installs is reported as what it is known to do.
	RiskUnverified RiskCategory = "unverified"
)

// AllRiskCategories is every gate-able category, in display order. Used to
// validate --allow and to render the help.
var AllRiskCategories = []RiskCategory{RiskFileDelete, RiskDataMutate, RiskNetEgress, RiskSysControl, RiskPkgInstall, RiskUnverified}
...
func (T *Servitor) Init() error {
	...
	T.Flags.StringVar(&T.input.allow, "allow", "", "Comma-separated risk categories to run WITHOUT confirmation (your \"levels of allowance\"): file_delete, data_mutate, net_egress, sys_control, pkg_install, unverified (commands the gate cannot confirm are read-only: unknown programs, scripts, interpreters). \"all\" allows every category. Example: --allow=net_egress,sys_control leaves the rest prompting. Default: every category prompts.")
...
func parse_allow(spec string) (map[RiskCategory]bool, error) {
	...
	for _, raw := range strings.Split(spec, ",") {
		...
		if !valid {
			return nil, fmt.Errorf("unknown --allow category %q (valid: file_delete, data_mutate, net_egress, sys_control, pkg_install, unverified, all)", name)
		}
	}
	return set, nil
}
...
func (T *Servitor) Main() error {
	...
	run_tool := AgentToolDef{
		...
		Handler: func(ctx context.Context, args map[string]any) (string, error) {
			...
			// Every risk on the line must be allowed, not just the first.
			cat, reason := risk_needing_approval(assess_command(cmd, scratch), T.cmd_allowed)
			if cat != RiskNone {
