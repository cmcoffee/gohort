// Bridge between servitor's probeEvent stream and the AgentLoopPanel
// SSE protocol. runSession emits probeEvents into a per-session queue
// (probeSessions); this file's handler taps that queue, translates each
// event into the shape AgentLoopPanel understands, and ships them
// over SSE.
//
...
// SSE event shape. Returns the map ready for sse.Send (or nil to drop
// the event silently).
//
// Mapping:
//
...
func translateProbeEvent(ev probeEvent) map[string]any {
	...
	case "status":
		...
		// overwritten on each new event. Connection chatter
		// ("Connected.", "SSH reconnected.") is noise once the
		// first command output flows; drop it entirely instead
		// of giving it a top-of-pane status bar to itself.
...
func (T *Servitor) handleChatEvents(w http.ResponseWriter, r *http.Request) {
	...
	// Nil return from the translator = "drop silently" (e.g. SSH
	// connection chatter that we don't want surfacing to the user).
