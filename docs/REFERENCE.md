...
- **Paged, searchable tool output** — a reply capped so one call cannot fill the context now hands the agent a way to the rest instead of stopping. `fetch_knowledge_doc` takes an `offset` and a `grep`; `run_command` keeps a capture that overflowed and serves later windows by `output_id`, with `grep` + `context` over the same capture — so reading on, or finding one line in a 60,000-character log, costs no second command and no workspace file. A grep hit carries the character offset of its line, so following it is one more call. See `core/paged_output.go`.
