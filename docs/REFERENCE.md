...
### Authoring surface
- **Chat-is-authoring**: Builder is the front door. Tell it "make me an agent for X" or "set up a workflow that does Y" and it creates/updates/clones/deletes the right thing. Runtime-enforced exclusivity keeps authoring out of other agents. A new session opens on **starting points** (Agent / App / Tool / Pipeline / Fix something) with the normal composer still live beside them: a one-line bug report stays a one-line bug report. Builder is dispatchable from channel/fleet agents, and from any agent granted **Can dispatch Builder** (Security & Access); the same starting points are published to callers as a brief checklist, since an under-specified brief is how a delegated authoring run goes wrong.
- **Four primitives** to compose: **agents** (persona + tool surface), **skills** (conditional prompt addendums with optional vector corpus that self-trains via post-turn closer), **collections** (RAG-attachable document buckets, deployment-wide or per-user), and **pipelines** (declarative multi-stage workflows, author once, then attach to any agent as a callable tool *or* mount as a page that submits a run, streams its stages, and keeps its history).
- **Pipelines**, a saved, ordered stage list with six stage kinds: **worker** (a plain LLM step), **agent** (a dispatch to one of your agents), **fanout** (breadth, runs in parallel across an earlier stage's list and collects the results), **loop** (depth, repeats a body of stages, each pass seeing the last via `{prev}`, bounded by `count` and optionally cut short by an `until` field), **branch** (control flow, reads a declared bool and either ends the pipeline or skips forward), and **tool** (calls one of your tools directly with arguments you write, no model, no tokens; the escape hatch for arithmetic, dedup, normalization, one specific API call). Outputs thread forward via `{input}`/`{prev}`/`{stage:NAME}`/`{stage:NAME.field}`/`{item}`/`{iteration}` templating.
- **Structured stage outputs**: any stage can declare its result shape (`output: [{name, type, desc, required}]`, types `string|number|bool|list|object`). The interpreter appends a field contract to the prompt, asks for JSON, validates it, and exposes each field downstream as `{stage:NAME.field}`, so a later stage can read *one piece* of an earlier result rather than its whole text. This is what makes the other primitives expressible: a `loop`'s `until` and a `branch`'s `when` both read a declared bool, and `fan_over` can point at a single list field. A stage that misses its declared shape gets one repair retry, then fails at the source rather than leaking an unresolved placeholder three stages downstream.
- **Per-stage model tier**: `model: "worker"` (default) or `"lead"`, the declarative equivalent of the route-stage keys compiled apps register. Put the precision tier on decomposition, synthesis, and judge stages; leave transforms and per-item fetches local. Rejected (not silently ignored) on the kinds it can't apply to.
- Author pipelines from chat via Builder, attach to an agent with `attached_pipelines` (it surfaces as a callable `run_<pipeline>` tool), run it, export/import the recipe as portable JSON, and view/delete from the admin panel. A pipeline can also BE an app: core serves the run surface (`core/pipeline_runs.go`, store a run, stream it over SSE in the shape `PipelinePanel` speaks, list past ones), and an `app_def` `pipeline` section mounts it at `/custom/<slug>/`. The submit form's fields become `{field_name}` in every stage's prompt, so a run takes parameters rather than one question. Only *agent* stages need the agent runtime: core takes that as a dispatch hook and stays ignorant of agents entirely. The recipe is plain data with no identity baked in, so it travels. Every reference (a `{stage:...}`, a `fan_over`, an `until`, a `skip_to`), is checked when the pipeline is **saved**, so a typo is an authoring error rather than a mid-run surprise.
- **Skill playbooks**, a skill can carry declared conditional behavior: "establish Y first; if yes do Z, otherwise U". The framework runs the check itself before the model's first round (the rule compiles to a one-phase machine carrying the skill's tools, with the fact as a declared output), and hands the agent only the arm that applies, so the condition is settled before either branch can start. Authored by question in Extensions › Skills, as JSON, or by Builder. See `docs/skill-playbooks.md`.
...
- **Paged, searchable tool output**: a reply capped so one call cannot fill the context now hands the agent a way to the rest instead of stopping. `fetch_knowledge_doc` takes an `offset` and a `grep`; anything else that overflows keeps its capture and serves later windows by `output_id`, with `grep` + `context` over the same capture, so reading on, or finding one line in a 60,000-character log, costs no second command and no workspace file. A grep hit carries the character offset of its line, so following it is one more call. One framework tool, `read_output`, speaks that id for every producer, so a truncated sub-agent report, delegated run or API body is reachable without teaching five tools their own paging arguments. See `core/paged_output.go`.
- **Releasing a spent result**: the other half of paging, and the one place the agent gets a say in what it carries. Compaction decides what leaves the conversation on size and age alone, and cannot know which of two large results is still being worked from. `release_output` names a capture the agent is finished with: every copy of it in the conversation becomes a one-line note holding the handle, and the capture itself is untouched, so `read_output` on the same id brings any part of it back. Only spilled results are releasable, which is the point rather than a limit: a result whose text exists nowhere else cannot be dropped without losing it. The agent loop acts on the call by name, the way it acts on `stay_silent`, so it works in every loop the framework runs rather than only the ones an app wired. Both tools ride along on every agent turn and survive a machine phase's narrowing, since reading a result that already arrived reaches nothing.
- **Per-session diagnostics trail**: every guard that suppresses, discards, retries, or rewrites something on the user's behalf leaves a bounded breadcrumb behind the chat's ⚠ affordance, so "what went wrong" is answerable from the UI instead of the server log. A **tool-churn detector** adds a card when a turn spends its calls without moving (the same tool with identical arguments three times, or one tool dominating a turn alongside errors), and a **Copy** button exports the trail as plain text to hand to whoever is debugging the framework: timestamps, kinds, and framework-authored sentences only, so there is nothing to redact.
...
- **Cross-user sharing**, owners publish resources to other authenticated users without copying them: **Scribe documents** (view or edit, with source-grounded read-only Q&A for readers), **Servitor Systems** (operated in the owner's scoped context), and **persistent tools** (an admin marks a tool shared → it loads for every user's agents). Shared records resolve to the owner's store through one generic `core/sharing` seam, so each user keeps their own sessions + memory and the owner's linked knowledge stays reachable only through the agent, never directly.
...
| `scribe` | The writing app: single-body **articles** and living multi-section **guide documents**, co-authored with an AI Guide Author. A guide renders as a formatted document (table of contents, per-section edit/move/delete); an article is one markdown body with an optional generated header image, typed into directly or written by the co-author. Shared: revision history, PDF / HTML / Markdown export, publish-out, knowledge collections + cross-app Sources, freshness audit, the Curator, and per-user house-style rules. Absorbed `guides` and `techwriter` (both paths redirect here; TechWriter libraries migrate in on first start) |
...
| `publish` | Where finished documents go: a registry of publish destinations (Confluence, webhook) that any document surface can target through `core/docs` |
...
| `prompts` | Edit the framework prompt blocks that shape agent behavior, without a rebuild |
...
│   ├── scribe/              # Writing app: articles + living guides with an AI co-author
