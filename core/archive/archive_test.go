...
func TestExpandArchivesUnpacksAndRemovesTheArchive(t *testing.T) {
	...
	res, err := Expand(context.Background(), dir, Limits{})
	if err != nil {
...
func TestExpandArchivesEnforcesTheByteBudget(t *testing.T) {
	...
	res, err := Expand(context.Background(), dir, Limits{MaxBytes: 512})
	if err == nil {
...
func TestExpandArchivesNestsToTheDepthCap(t *testing.T) {
	...
	if _, err := Expand(context.Background(), dir, Limits{}); err != nil {
		t.Fatalf("expand: %v", err)
	}
	...
	if _, err := Expand(context.Background(), dir2, Limits{MaxDepth: 1}); err != nil {
		t.Fatalf("expand: %v", err)
	}
...
func TestSafeJoinAndUnopened(t *testing.T) {
	...
	if Unopened("app.log") || Unopened("x.tar.gz") {
		t.Error("a plain file or a supported archive is not 'unopened'")
	}
	if !Expandable("x.tar.gz") || Expandable("app.log") {
		t.Error("Expandable disagrees with the expander table")
	}
}
