...
func (m *MigrationRunner) Once(name string, fn func() int) {
	...
	if panicErr != "" {
		nfo.Log("[migration] %s/%s PANIC for %s: %s", m.app, name, ownerLabel(m.owner), panicErr)
		return
	}
	if changed > 0 {
		nfo.Log("[migration] %s/%s: %d record(s) changed for %s", m.app, name, changed, ownerLabel(m.owner))
	}
}
