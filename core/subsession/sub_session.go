...
// MostRecentActiveSubSession returns the newest still-active sub-session for a
// host session, or nil. Lives here rather than in the promotion logic that
// calls it: it reads this package's table under this package's lock.
func MostRecentActiveSubSession(hostSessionID string) *SubSession {
	if rootDB() == nil {
		return nil
	}
	subSessionMu.Lock()
