...
func SaveChannel(db Store, ch Channel) {
...
func GetChannel(db Store, owner, id string) (Channel, bool) {
	if db == nil || owner == "" || id == "" {
...
func ListChannels(db Store, owner string) []Channel {
	if db == nil || owner == "" {
		return nil
	}
	prefix := owner + ":"
...
func ListChannelsForAgent(db Store, owner, agentID string) []Channel {
	var out []Channel
...
func ChannelAllowsSender(db Store, owner, chatID, handle, agentID string) bool {
	if agentID == "" {
		return false
	}
	for _, ch := range ListChannels(db, owner) {
...
func DeleteChannel(db Store, owner, id string) {
	if db == nil {
		return
	}
...
func ChannelForInbound(db Store, owner, service string, addresses ...string) (Channel, bool) {
