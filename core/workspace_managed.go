...
// workspace. If sess.WorkspaceDir is already set (the typical case
// — apps populate it with the per-user root at session creation),
// it is returned unchanged. Otherwise an unnamed ephemeral managed
// workspace is minted, the session's WorkspaceDir/WorkspaceID are
// populated, and the new dir is returned.
//
// Used by tools that need a sandbox to operate in (local read/
// write/run, tool_def create with mode=shell) so the LLM never has
// to explicitly mint a workspace before its first useful call.
//
// Requires sess.Username to be set when auto-mint fires — managed
// workspaces are owner-scoped. ChatSessionID / RoutingTarget are
// used as the originating session ID when available.
