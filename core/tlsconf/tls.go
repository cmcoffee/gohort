...
func resolveTLSFiles() (cert_file, key_file string, err error) {
	...
	data_dir := filepath.ToSlash(filepath.Join(filepath.Dir(exec), "data", "tls"))
	...
	if fileExists(cert_file) && fileExists(key_file) {
		if !certExpired(cert_file) {
			nfo.Log("Using cached self-signed certificate from %s", data_dir)
			return cert_file, key_file, nil
		}
		nfo.Log("Self-signed certificate expired, regenerating.")
	}

	nfo.Log("Generating self-signed TLS certificate...")
...
func generateSelfSigned(cert_file, key_file string) error {
	...
	nfo.Log("Self-signed certificate generated: %s", cert_file)
	return nil
}
...
// SelfSigned reports whether self-signed generation is enabled. An accessor
// because the var behind it is mutable config: core.go can re-export this
// call, but a var alias would freeze whatever value startup hadn't set yet.
func SelfSigned() bool { return TLSSelfSigned }
