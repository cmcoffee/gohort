...
type ExtraSessionsSource interface {
	...
	ListForAgent(db Store, agentID, user string) []ExtraSessionItem
	...
	LoadSession(db Store, agentID, user, sessionID, chatID string) (any, bool)
}
...
func CollectExtraSessions(db Store, agentID, user string) []ExtraSessionItem {
	extraSessionsMu.RLock()
