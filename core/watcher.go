...
type Watcher struct {
	...
	// Description is a human-readable one-liner explaining what the
	// watcher exists to detect. Distinct from ActionPrompt (which is
	// what the worker DOES when a change fires); description answers
	// "what is this watcher FOR?". Optional but strongly encouraged —
	// without it, list views can only show the short Name and the
	// captured tool name, making it hard for the LLM (or the operator)
	// to disambiguate at delete time.
