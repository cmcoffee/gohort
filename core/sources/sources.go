...
func (sr *SourceRegistry) ExtractCitedURLs(text string) []string {
	...
	sr.mu.Lock()
	defer sr.mu.Unlock()
	for _, match := range textutil.CiteRefPattern.FindAllStringSubmatch(text, -1) {
		n := 0
...
func RewriteCitations(text string, remap map[int]int) string {
	return textutil.CiteRefPattern.ReplaceAllStringFunc(text, func(match string) string {
		var n int
