...
func TestTheSecondCallOfATurnStartsNoSecondJob(t *testing.T) {
	...
	first, err := def.Handler(context.Background(), map[string]any{"prompt": "blend Alex onto the garage photo"})
	if err != nil {
	...
	second, err := def.Handler(context.Background(), map[string]any{"prompt": "put Alex on the hood instead"})
	if err != nil {
