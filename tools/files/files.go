...
// are no longer registered as their own catalog entries — the merged
// `workspace` grouped tool (tools/workspace/workspace.go) wires them
// in as ls / cat / write actions. Their implementations remain
// exported so the workspace package can reuse them without duplication.
func init() {}
