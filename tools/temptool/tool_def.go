...
func BuildToolDef() *GroupedTool {
	...
	gt.AddAction("create", &GroupedToolAction{
		...
		Params: map[string]ToolParam{
			...
			"credential":        {Type: "string", Description: "(api / toolbox mode, optional) Name of the registered secure-API credential to dispatch through. For public no-auth APIs, OMIT it — it defaults to \"no_auth\", the bootstrapped open-pattern credential that applies gohort's allow-list/audit/rate-limit but injects no auth header. If you name the no-auth case explicitly, \"no_auth\" is the ONLY accepted spelling — never placeholders like \"none\" / \"public\" / \"n/a\". For authenticated APIs, name the credential the admin registered (\"github\", \"openweather\", etc.); the secret stays server-side and gohort injects the header (Bearer / custom / etc.) at dispatch. REUSE the existing credential for a service — it's normally named after the service (a Moltbook tool → credential \"moltbook\"). Do NOT mint a duplicate, and never solicit an API key in chat. If dispatch reports the credential DISABLED, ask the user to enable it rather than routing around it or recreating — a disabled credential blocks dispatch by design. If the credential you need doesn't exist, point the user at Builder to set it up."},
			...
			"body_template":     {Type: "string", Description: "(api mode) Request body template with {param} placeholders. JSON by default: placeholders are JSON-encoded (strings auto-quoted) and the result is JSON-validated. For a non-JSON body set content_type — placeholders then substitute raw, with no JSON validation. Optional for GET; usually required for POST/PUT/PATCH."},
			...
			"content_type":      {Type: "string", Description: "(api mode, optional) Content-Type for the request body. Empty = application/json. Any non-JSON value (e.g. \"application/xml\", \"text/calendar\") switches body_template to raw substitution, skips JSON validation, and sends that header. See tool_def(action=\"help\")."},
			...
			"actions": {Type: "array", Description: "(toolbox mode) Sub-action endpoints. Array of objects, each shape: {name, description, url_template, params, required?, method?, body_template?, content_type?, response_pipe?}. Every action is one api-mode endpoint with its own params + URL. Names must be unique within the toolbox; the LLM calls the toolbox as <toolbox_name>(action=\"<sub>\", ...). The toolbox's top-level credential is shared across all actions — APIs almost always have one credential per service. Actions can mix body formats: set content_type per action (e.g. application/xml for a CalDAV REPORT, text/calendar for a PUT) so each sends a RAW non-JSON body. NOT for shell-mode sub-actions (toolbox is api-only today); use mode=\"shell\" for those.",
				Items: &ToolParam{
					...
					Properties: map[string]ToolParam{
						...
						"body_template":    {Type: "string", Description: "Optional request body template. {param} placeholders are JSON-encoded and the result JSON-validated by default; set content_type (below) for a non-JSON body, which substitutes raw instead."},
						...
						"content_type":     {Type: "string", Description: "Optional Content-Type for this action's body. Empty = application/json. A non-JSON value switches body_template to raw substitution, skips JSON validation, and sends that header."},
