...
// is the canonical agent-dispatch surface. DelegateTool's standalone
// registration was creating a fourth way to do agent dispatch and
// confusing the LLM's catalog. The struct + methods stay so any
// stale agent records naming "delegate" in AllowedTools don't crash
// — they just resolve to nothing and the agent uses `agents` instead.)
...
func (t *DelegateTool) IsFrameworkTool() bool { return true } // hide from admin tool-groups picker — framework/dispatch surface, not a user-groupable capability
...
// Caps tags delegate as CapNetwork even though the bare call doesn't
// make HTTP requests — the workers it dispatches inherit the full
// catalog (web_search, fetch_url, etc.) by default, so a Private-
// mode turn that had delegate visible could still leak via the worker
// tier. Same rationale as the orchestrate `agents` grouped tool.
