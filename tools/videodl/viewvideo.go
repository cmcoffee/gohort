...
// ViewVideoTool's standalone registration is dropped — surfaces via
// the grouped `video` tool (video(action="view")). Type + RunWithSession
// stay public so the grouped dispatcher invokes it. Without this
// consolidation the LLM kept reaching for view_video on URL pastes
// (single-call, no follow-up attach needed) and never invoked the
// delivery path — even when the user clearly wanted the file back.
