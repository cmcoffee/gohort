...
// AttachFileTool is no longer registered as its own catalog entry —
// the `workspace` grouped tool now owns delivery via action="attach".
// Implementation kept around in case any test / private caller still
// reaches for it directly, but no global registration.
func init() {}
