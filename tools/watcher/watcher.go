...
func init() {
	...
	gt.AddAction("create", &GroupedToolAction{
		Description: "Create a polling watcher around a tool call. Required: name, description, tool_name, tool_args, interval_seconds (>=60). Pick ONE evaluator: \"llm\"+action_prompt (worker LLM analyzes each diff), \"script\"+evaluator_script+evaluator_test (sandboxed python — zero LLM cost), or \"raw\" (current response IS the alert). Call action=\"help\" for the recommended flow + script-writing rules.",
		Params: map[string]ToolParam{
			"name":             {Type: "string", Description: "Short identifier (snake_case)."},
			"description":      {Type: "string", Description: "One-line human-readable purpose — what this watcher exists to detect (e.g. \"alert when a new TS3 client joins\"). Shown in list/peek so future calls can disambiguate which watcher to update/delete. Distinct from action_prompt (which is what to DO on change)."},
		...
		Required:     []string{"name", "description", "tool_name", "tool_args", "interval_seconds"},
	...
	gt.AddAction("update", &GroupedToolAction{
		Description: "Update an existing watcher's description, action_prompt, interval_seconds, evaluator, evaluator_script, or delivery_prefix. Cannot change the captured tool_name/tool_args (changing what's being watched is structurally a different watcher — delete + create instead). Returns the updated watcher record.",
		Params: map[string]ToolParam{
			"id":               {Type: "string", Description: "Watcher ID (from list)."},
			"description":      {Type: "string", Description: "Optional. New human-readable purpose. Omit to leave unchanged."},
...
func handleList(args map[string]any, sess *ToolSession) (string, error) {
	...
	for _, w := range ws {
		...
		b.WriteString("\n")
		// Purpose line: prefer the explicit Description; fall back to the
		// action_prompt for legacy llm-evaluator watchers that pre-date
		// the Description field. Compact tool_args inline so the LLM can
		// tell two watchers on the same tool apart at a glance.
		purpose := strings.TrimSpace(w.Description)
		if purpose == "" {
			purpose = strings.TrimSpace(w.ActionPrompt)
		}
		if purpose != "" {
...
func handleCreate(args map[string]any, sess *ToolSession) (string, error) {
	...
	if description == "" {
		return "", fmt.Errorf("description is required — one-liner explaining what this watcher exists to detect (used in list/peek to disambiguate at delete time)")
	}
	if interval < 60 {
...
func handleUpdate(args map[string]any, sess *ToolSession) (string, error) {
	...
	if v := strings.TrimSpace(StringArg(args, "description")); v != "" {
		w.Description = v
	...
	if len(changed) == 0 {
		return "No changes — pass at least one of description, action_prompt, interval_seconds, evaluator, evaluator_script, delivery_prefix.", nil
	}
...
func handlePeek(args map[string]any, sess *ToolSession) (string, error) {
	...
	if p := strings.TrimSpace(w.ActionPrompt); p != "" {
		fmt.Fprintf(&b, "  action_prompt: %s\n", p)
	}
