...
| `guides` | Living multi-section **guide documents** co-authored with an AI Guide Author: markdown sections rendered as a formatted document (table of contents, revision history, PDF / HTML / Markdown export); attach knowledge collections + cross-app reference sources to ground drafting; **source-grounded Audit** + one-click **Update from sources** (re-runs the author over sections against current sources, each edit a revision). **Shareable** view or edit to other users — a reader gets read-only Q&A answered from the guide's linked knowledge and sources, resolved in the owner's context, that they cannot reach directly |
